#!/usr/bin/env python3
"""Assert every non-draft page's translationKey exists in both content/en and content/pt-br.
...
def collect_keys(content_dir: Path, lang: str) -> dict:
    """Map translationKey -> page path for non-draft pages in one language.

    It walks the language directory RECURSIVELY rather than one blog section,
    because a product site keeps its pages at the root and grows sections
    (pricing, legal, docs) that nobody should have to register here first. A
    checker that only sees one folder stops checking the moment somebody adds
    another, and stops silently — which is the failure this file exists to
    prevent.
    """
    keys = {}
    posts_dir = content_dir / lang
    for post in sorted(posts_dir.rglob("*.md")) if posts_dir.is_dir() else []:
