...
make_fixture() { # $1=dir
  mkdir -p "$1/en/writing" "$1/pt-br/artigos" "$1/en/legal" "$1/pt-br/legal"
}
...
# A page OUTSIDE the blog section must be checked too. Before the walk became
# recursive this passed while checking nothing, which is the exact bug the
# recursion fixes: a checker that goes quiet as the site grows.
FX_NESTED=$(mktemp -d)
make_fixture "$FX_NESTED"
mkdir -p "$FX_NESTED/en/legal" "$FX_NESTED/pt-br/legal"
write_post "$FX_NESTED/en/legal/privacy.md" privacy false
check "nested page with no pair fails" 1 "$FX_NESTED"
